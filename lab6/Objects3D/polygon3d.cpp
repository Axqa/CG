...
QColor Polygon3D::GetColor(Camera &cam)
{
    if (calcColor) {
        return NormToColor(cam);
    } else {
        return color;
    }
}
...
QColor Polygon3D::NormToColor(Camera &cam)
{
    int s = 1;

    if (plane.IsOnPositiveSide(cam.plane.normal*10000)) {
        s = -1;
    }

    QColor col;

    float lum = cam.scene->ComputeLightForPointOnPlane(center, plane);

    int r = (127 + s*plane.normal.y * 127) * lum;
    int g = (127 + s*plane.normal.x * 127) * lum;
    int b = (127 + s*plane.normal.z * 127) * lum;
...
{
    cam.sImage.FillPolygon(points.toStdVector(), GetColor(cam));
}
...
QGraphicsItemGroup *Polygon3D::DrawOnCameraView(Camera &cam)
{
    ...
    prPol.FromMatrix(pr);
    if (calcColor)
        prPol.SetColor(NormToColor(cam));
    else
